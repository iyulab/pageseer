...
**결정 유보:** `pyo3`/`napi-rs` 등 언어별 FFI crate 사용 여부. v0.1 말기에 C ABI 경계 안정성이 확보되면 언어별 경로로 분기 가능성 재평가.
...
- **페이지 단위 병렬화** — pdfium thread_safe 내부 mutex 개선 여부에 따라 재평가
